package com.kw.app.commonlib.utils.luban;
...
import static com.kw.app.commonlib.utils.luban.Preconditions.checkNotNull;
...
public class Luban {
    ...
    public static String DEFAULT_DISK_CACHE_DIR = AppConstant.CACHE_PATH;
    ...
    public static File getPhotoCacheDir() {
        return getPhotoCacheDir(DEFAULT_DISK_CACHE_DIR);
    }
     ...
     */
    public static File getPhotoCacheDir(String cacheName) {

        File cacheDir = new File(cacheName);//默认路径
        if(!cacheDir.exists()){
            cacheDir.mkdirs();
        }
    ...
    public static Luban get() {
        if (INSTANCE == null) INSTANCE = new Luban(getPhotoCacheDir());
        return INSTANCE;
    }

    public Luban launch() {
        checkNotNull(mFile, "the image file cannot be null, please call .load() before this method!");
        checkNotNull(mCacheDir,"the cache path cannot be null,please check it.");
    ...
    private File thirdCompress(@NonNull File file) {
        String thumb = mCacheDir.getAbsolutePath() + File.separator + (TextUtils.isEmpty(filename) ? System.currentTimeMillis() + ".jpeg" : filename);
