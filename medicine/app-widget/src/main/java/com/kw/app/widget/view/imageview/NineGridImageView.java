package com.kw.app.widget.view.imageview;
...
import com.kw.app.widget.adapter.NineGridImageViewAdapter;
...
public class NineGridImageView<T> extends ViewGroup {
    ...
    public void setImagesData(List lists) {
        ...
        if (mImgDataList == null) {
            ...
            //NineGridImageView 进行回收
            int oldViewCount = mImgDataList.size();
            int newViewCount = lists.size();
            if (oldViewCount > newViewCount) {
                //如果旧的图片个数大于新的图片个数就去掉，就去掉多余的
                removeViews(newViewCount, oldViewCount - newViewCount);
            } else if (oldViewCount < newViewCount) {
                //如果旧的imageview不够显示，就创建新的
    ...
    protected LayoutParams generateDefaultLayoutParams() {
        return new LayoutParams(mGridSize, mGridSize);
    }
